...
\textbf{Problem 1} What are the upper $3 \ensuremath{\times} 3$ sub-block of the Jacobi matrix for the  monic and orthonormal polynomials with respect to the following weights on $[-1,1]$:
\[
...
\textbf{Problem 6(a)} For the matrix
\[
...
\]
use the relationship with the Jacobi matrix associated with $T_n(x)$ to prove that
\[
...
for $x_j = \cos \ensuremath{\theta}_j$, and $\ensuremath{\theta}_j = (n-j+1/2)\ensuremath{\pi}/n$ for $j = 1,\ensuremath{\ldots},n$. You may use without proof the sums-of-squares formula
\[
1 + 2 \ensuremath{\sum}_{k=1}^{n-1} \cos^2 k \ensuremath{\theta}_j = n.
\]
\textbf{Problem 6(b)} Show for $w(x) = 1/\sqrt{1-x^2}$ that the Gaussian quadrature rule is
\[
...
\textbf{Problem 6(c)} Give an explicit formula for the polynomial that interpolates $\exp x$ at the points $x_1,\ensuremath{\ldots},x_n$ as defined above.
