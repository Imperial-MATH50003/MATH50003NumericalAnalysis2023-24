...
\]
Hint: You may wish to try the change of variables $z = {\rm e}^{-{\rm i}\ensuremath{\theta}}$.
...
\end{bmatrix}
\]
for $\ensuremath{\theta}_j = \ensuremath{\pi}(j-1/2)/n$. Prove that $C_n$ is orthogonal: $C_n^\ensuremath{\top} C_n = C_n C_n^\ensuremath{\top} = I$. Hint: $C_n C_n^\ensuremath{\top} = I$ might be easier to show than $C_n^\ensuremath{\top} C_n = I$ using the previous problem.
