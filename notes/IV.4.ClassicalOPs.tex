...
\end{itemize}
As stated above orthogonal polynomials are uniquely defined by the weight $w(x)$ and the constant $k_n$. The classical orthogonal polynomials are:
...
\item[2. ] Legendre polynomials: These have no simple closed-form expression but can be defined in terms of a Rodriguez formula, a feature that
...
\end{align*}
\end{corollary}
\textbf{Proof} This is rewriting the expression we used to show that $p_n(x0$ are graded. \ensuremath{\QED}
