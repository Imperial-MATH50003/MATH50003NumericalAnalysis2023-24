...
\subsection{Rectangular rule error bound}
We can use bounds on floating point arithmetic to derive bounds on the error in a more complicated computation. For example, consider our  rectangular rule approximation
\[
\int_0^1 f(x) {\rm d} x = {1 \over n} \sum_{j=1}^n f(j/n) + \ensuremath{\delta}
\]
Assume for simplicity that $n = 2^s$, i.e., a power of 2, so that in idealised floating point we have $x \ensuremath{\oslash} n = x / n$ for any $x \ensuremath{\in} F_{\ensuremath{\infty},S}$. Thus our actual approximation is
\[
\int_0^1 f(x) {\rm d} x = {1 \over n} \ensuremath{\bigoplus}_{j=1}^n f^{\rm FP}(j/n) + \ensuremath{\delta}^{\rm FP}
\]
where
\[
f^{\rm FP}(x) = f(x) + \ensuremath{\varepsilon}_x^f
\]
represents the function $f$ implemented in floating point, that is $f^{\rm FP} : F_{S,\ensuremath{\infty}} \ensuremath{\rightarrow} F_{S,\ensuremath{\infty}}$ and assume we have a bound of the form $|\ensuremath{\varepsilon}_x^f| \ensuremath{\leq} c \ensuremath{\epsilon}_{\rm m}$. 
