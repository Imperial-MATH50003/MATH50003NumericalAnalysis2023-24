...
Suppose $x_1, \ensuremath{\ldots},x_j$ are the roots where $q_n(x)$ changes sign, i.e., the order of the root must be odd and hence
\[
...
\end{bmatrix}
\]
The sepctral theorem guarantees that all symmetric matrices have an orthogonal eigenvector matrix.  That is, by scaling the columns of the eigenvectors we know there must exist $\ensuremath{\alpha}_j$ so that 
\[
Q_n = \underbrace{\begin{bmatrix}
q_0(x_1) & \ensuremath{\cdots} & q_0(x_n) \\
\ensuremath{\vdots}  & \ensuremath{\cdots} & \ensuremath{\vdots}  \\
